...
1. **多伦多油价走势 + 预测**：使用保存在 `data/gas-prices.json` 里最近一个月的 Toronto regular gas 每日均价（CAD c/L）画成走势图，并突出显示 CityNews / En-Pro 发布的次日油价。
2. **RedFlagDeals 热门 deal**：读取 `data/rfd-deals.json`，按 heat 排序，最多显示 12 条。

## 数据来源

- Gas: `data/gas-prices.json`
  - 历史数据来自 CityNews Toronto 的 Historical Values 表。
  - 次日预测保存在 `data/gas-forecast.json`，来自 CityNews / En-Pro。
  - GitHub Actions 每天运行 `collect_gas.py --fetch`，自动维护滚动 30 天数据。
...
- 数据收集和网页展示分离：
  - cron job / 后台脚本负责更新 `data/gas-prices.json`、`data/gas-forecast.json` 和 `data/rfd-deals.json`
  - HTML/JS 只读取这些 JSON 文件渲染，不需要后端服务
- 每天的 GitHub Actions 更新数据并 push 后，静态页面刷新即可看到最新走势图。
